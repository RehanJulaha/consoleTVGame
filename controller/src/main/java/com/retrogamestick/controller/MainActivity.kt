...
import retrofit.gamestick.network.*

class MainActivity : ComponentActivity() {
    private val TAG = "ControllerMainActivity"
    
    private lateinit var networkManager: ControllerNetworkManager
    ...
    // D-Pad state
    private val dPadState = remember { mutableStateOf(DPadState()) }
    
    // Buttons state
    private val buttonsState = remember { mutableStateOf(ButtonsState()) }
    
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        
        networkManager = ControllerNetworkManager(
            context = this,
            scope = scope,
            onConnected = { slot, session ->
                isConnected = true
                myPlayerSlot = slot
                sessionId = session
            },
            ...
            onPinRequired = { pin ->
                runOnUiThread { showPinDialog(pin) }
            },
            onError = { msg ->
                Log.e(TAG, msg)
            }
        ...
        setContent {
            RetroGameStickControllerTheme {
                ControllerScreen(
                    ...
                    onInputChange = { frame ->
                        if (isConnected) {
                            networkManager.sendInputFrame(frame)
                        }
                    }
                )
            }
...
fun ControllerScreen(
    ...
    onInputChange: (InputFrame) -> Unit
) {
    val frameCounter = remember { mutableStateOf(0u) }
